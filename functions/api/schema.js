  ...
  try {
    ...
    const statements = [
      ...
      `CREATE TABLE IF NOT EXISTS payment_instructions (
        ...
        recipient_count INT NOT NULL DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())`,
      `CREATE TABLE IF NOT EXISTS payment_instruction_lines (
        ...
        bank_code TEXT, masked_account TEXT NOT NULL, account_ciphertext TEXT, account_iv TEXT,
        account_last4 TEXT, line_hash TEXT, amount BIGINT NOT NULL)`,
    ...
    const alters = [
      ...
      `ALTER TABLE payment_instructions ADD COLUMN IF NOT EXISTS document_no TEXT`,
      `ALTER TABLE payment_instructions ADD COLUMN IF NOT EXISTS content_hash TEXT`,
      `ALTER TABLE payment_instructions ADD COLUMN IF NOT EXISTS currency TEXT NOT NULL DEFAULT 'IDR'`,
      `ALTER TABLE payment_instructions ADD COLUMN IF NOT EXISTS execution_date DATE`,
      `ALTER TABLE payment_instructions ADD COLUMN IF NOT EXISTS recipient_count INT NOT NULL DEFAULT 0`,
      `ALTER TABLE payment_instruction_lines ADD COLUMN IF NOT EXISTS bank_code TEXT`,
      `ALTER TABLE payment_instruction_lines ADD COLUMN IF NOT EXISTS account_ciphertext TEXT`,
      `ALTER TABLE payment_instruction_lines ADD COLUMN IF NOT EXISTS account_iv TEXT`,
      `ALTER TABLE payment_instruction_lines ADD COLUMN IF NOT EXISTS account_last4 TEXT`,
      `ALTER TABLE payment_instruction_lines ADD COLUMN IF NOT EXISTS line_hash TEXT`,
      ...
      `WITH ranked AS (SELECT id, ROW_NUMBER() OVER (PARTITION BY employee_id ORDER BY created_at DESC, id DESC) AS rn FROM employee_bank_accounts WHERE is_primary=TRUE) UPDATE employee_bank_accounts eba SET is_primary=FALSE FROM ranked r WHERE eba.id=r.id AND r.rn>1`,
      ...
      `CREATE OR REPLACE FUNCTION prevent_payment_instruction_line_update() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'Payment instruction snapshot is immutable'; END; $$ LANGUAGE plpgsql`,
      `DROP TRIGGER IF EXISTS trg_payment_instruction_line_immutable ON payment_instruction_lines`,
      `CREATE TRIGGER trg_payment_instruction_line_immutable BEFORE UPDATE ON payment_instruction_lines FOR EACH ROW EXECUTE FUNCTION prevent_payment_instruction_line_update()`,
