import {
  authenticateEmployee, employeeHandlePreflight, employeeJson, passwordRecord,
  portalMutationAllowed, validatePassword, verifyPassword,
...
export async function onRequest({ request, env }) {
  ...
  if (problem) return employeeJson({ error: problem }, 422, request, env, METHODS);
  ...
  try {
    ...
    if (!current || !(await verifyPassword(currentPassword, current))) {
      return employeeJson({ error: 'Password saat ini tidak valid' }, 401, request, env, METHODS);
    }
    ...
    return employeeJson({ ok: true, sessionRevoked: true }, 200, request, env, METHODS);
  } catch (error) {
    return employeeJson({ error: 'Password change failed', ...publicError(error, crypto.randomUUID()) }, 500, request, env, METHODS);
  }
}
