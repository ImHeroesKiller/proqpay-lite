...
export async function onRequest(context) {
  ...
  const instructions = await sql`SELECT pi.*,s.period AS payroll_period,COALESCE(s.payment_period,s.period) AS payment_period,
    ...
    WHERE pi.id=${id} AND pi.org_id=${orgId(env)} LIMIT 1`;
  ...
  const lines = await sql`SELECT * FROM payment_instruction_lines WHERE payment_instruction_id=${id} ORDER BY employee_id,id`;
  const approvals = await sql`SELECT pa.*,au.email AS approver_email FROM payment_approvals pa
    LEFT JOIN app_users au ON au.id=pa.approver_user_id WHERE pa.payment_instruction_id=${id} ORDER BY pa.created_at`;
  if (format === 'PDF') {
  ...
  await sql`INSERT INTO audit_logs(id,org_id,username,role,action,detail,entity,entity_id)
    VALUES(${`AUD-${crypto.randomUUID()}`},${instruction.org_id},${authorization.actor.email},${authorization.actor.role},
      'PAYMENT_BANK_FILE_EXPORTED',${`${format} · ${instruction.content_hash}`},'payment_instruction',${id})`;
