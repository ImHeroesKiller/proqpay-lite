...
export function validateDirectoryAction(input) {
  ...
  if (input.startDate && input.endDate && String(input.endDate) < String(input.startDate)) errors.push('periode project tidak valid');
  ...
  const taxStatus = input.taxStatus || 'NON_PKP';
  const billingMethod = input.billingMethod || 'PER_EMPLOYEE';
  const rawBillingTaxRate = input.billingTaxRate === undefined ? 0 : Number(input.billingTaxRate);
  const billingTaxRate = taxStatus === 'PKP' ? rawBillingTaxRate : 0;
  ...
  if (billingMethod === 'PERCENTAGE_OF_PAYROLL' && billingRate > 100) errors.push('rate persentase payroll tidak valid');
  return { ok: errors.length === 0, errors, value: {
    ...
    paymentTermsDays, taxStatus, purchaseOrder: optionalText(input.purchaseOrder, 120),
    billingMethod, billingRate, billingAdminFee, billingTaxRate,
