...
const migration = readFileSync(
  "migrations/0017_employee_intake_history.sql",
  "utf8",
);
...
test("latest confirmed employee data updates current master while preserving history", () => {
  ...
  assert.match(
    api,
    /UPDATE employees SET client_id=\?,project_id=\?,employee_code=\?,name=/,
  );
...
test("backend revalidates intake scope and confirmation contract", () => {
  ...
  assert.doesNotMatch(api, /servicePlanId \|\| !context\?\.tier/);
});
