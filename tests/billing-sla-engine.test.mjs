...
test('migration is D1-safe, seeds 17 official 2026 holidays, and enforces one active SLA policy per scope', () => {
...
test('policy writes are idempotent and protected against concurrent active versions', () => {
  ...
  assert.match(migration, /idx_one_active_client_sla_policy/);
  assert.match(migration, /idx_one_active_project_sla_policy/);
});
...
test('AR materialization heals a concurrent duplicate instead of returning a 500', () => {
  assert.match(service, /UNIQUE constraint failed: ar_monitor\\\.invoice_id|ar_monitor\\\.invoice_id/);
