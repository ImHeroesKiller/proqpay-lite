...
function CreatePayRunWizard({clients,projects,servicePlans,submissions,onClose,onCreated}:{clients:any[];projects:any[];servicePlans:any[];submissions:any[];onClose:()=>void;onCreated:()=>Promise<void>}) {
  ...
  const adjustmentParentStates=new Set(['PAYROLL_FINALIZED','CLIENT_APPROVAL_PENDING','CLIENT_APPROVED','PAYMENT_INSTRUCTION_READY','PAYMENT_APPROVAL_PENDING','APPROVED_FOR_PAYMENT','DISBURSEMENT_PROCESSING','PROOF_UPLOADED','RECONCILIATION','PAYMENT_EXCEPTION','COMPLETED']);
  const parentRuns=submissions.filter((row)=>row.client_id===form.clientId&&row.project_id===form.projectId&&row.run_type==='REGULAR'&&row.period<=form.period&&adjustmentParentStates.has(String(row.state||'')));
...
function Submissions({ rows, instructions, role, permissions, simplified, act }: { rows: any[]; instructions:any[]; role: string; permissions:string[]; simplified:boolean; act: (p: Record<string, unknown>, s: string) => Promise<void> }) {
  ...
  function openReview(row:any) {
    ...
    setSelected(row); setConfirmed(false); setReviewNote(''); setWorkflowDialogOpen(false);
  ...
  useEffect(()=>{
    ...
    const frame=requestAnimationFrame(()=>reviewDialogRef.current?.querySelector<HTMLElement>('button,input,textarea,select')?.focus());
    const onKey=(event:KeyboardEvent)=>{
      if(event.key==='Escape'&&!workflowDialogOpen){event.preventDefault();setSelected(null);return;}
      if(event.key!=='Tab'||!reviewDialogRef.current) return;
      const items=[...reviewDialogRef.current.querySelectorAll<HTMLElement>('button:not([disabled]),a[href],input:not([disabled]),textarea:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])')];
  ...
  return <>{table}{createPortal(<div className="directory-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
    <div ref={reviewDialogRef} className="directory-modal payroll-review-modal" role="dialog" aria-modal="true" aria-label="Review payroll submission">
      ...
      {runDetailLoading?<div className="directory-hint">Menghitung variance terhadap periode sebelumnya…</div>:runDetail?<div className="pay-run-variance pay-run-variance-rich"><div><span>Periode pembanding</span><strong>{runDetail.previousPeriod||'Periode pertama'}</strong></div><div><span>Δ Headcount</span><strong>{Number(runDetail.variance?.headcountAmount||0)>0?'+':''}{runDetail.variance?.headcountAmount||0}</strong><small>{runDetail.variance?.currentHeadcount||0} penerima saat ini</small></div><div><span>Δ Gross</span><strong>{formatIDR(Number(runDetail.variance?.grossAmount||0))}</strong></div><div><span>Δ Potongan</span><strong>{formatIDR(Number(runDetail.variance?.deductionAmount||0))}</strong></div><div><span>Δ Net/THP</span><strong>{formatIDR(Number(runDetail.variance?.amount||0))}</strong><small>{runDetail.variance?.percent===null?'-':`${runDetail.variance.percent}%`}</small></div><div><span>New / Changed / Removed</span><strong>{runDetail.variance?.newEmployees||0} / {runDetail.variance?.changedEmployees||0} / {runDetail.variance?.removedEmployees||0}</strong></div><div><span>Rekening berubah</span><strong>{runDetail.variance?.bankChangedEmployees||0}</strong></div></div>:null}
      ...
      {workflowDialogOpen?<section className="pay-run-action-composer"><div><strong>{workflowDialog.kind==='REFRESH'?'Refresh dari Master':workflowDialog.kind==='DELETE'?'Hapus Pay Run':workflowDialog.kind==='REOPEN'?'Buka kembali periode':workflowDialog.kind==='CLOSE'?'Tutup periode':'Minta revisi'}</strong><button type="button" aria-label="Tutup action form" onClick={()=>setWorkflowDialogOpen(false)}>✕</button></div>{workflowDialog.kind==='REFRESH'?<p>Nominal, rekening, dan population akan disinkronkan ulang dari master saat ini. Input status kembali PENDING dan harus difinalisasi ulang.</p>:workflowDialog.kind==='CLOSE'?<p>Payroll/payment snapshot tetap immutable setelah periode ditutup. AR tetap dipantau terpisah.</p>:workflowDialog.kind==='DELETE'?<label>Ketik <strong>HAPUS PAY RUN</strong><input value={workflowDialog.value} onChange={(event)=>setWorkflowDialog({...workflowDialog,value:event.target.value})}/></label>:<label>Alasan tindakan<textarea rows={3} maxLength={500} value={workflowDialog.value} onChange={(event)=>setWorkflowDialog({...workflowDialog,value:event.target.value})} placeholder="Minimal 10 karakter" /></label>}<div><button type="button" className="btn" onClick={()=>setWorkflowDialogOpen(false)}>Batal</button><button type="button" className="btn btn-primary" disabled={(workflowDialog.kind==='DELETE'&&workflowDialog.value!=='HAPUS PAY RUN')||(['REOPEN','CONTROLLER_REVISION','CLIENT_REVISION'].includes(workflowDialog.kind)&&workflowDialog.value.trim().length<10)} onClick={()=>{const value=workflowDialog.value.trim();const payload=workflowDialog.kind==='DELETE'?{action:'DELETE_PAY_RUN',submissionId:selected.id,confirmation:'HAPUS PAY RUN'}:workflowDialog.kind==='REFRESH'?{action:'REFRESH_PAY_RUN_FROM_MASTER',submissionId:selected.id}:workflowDialog.kind==='REOPEN'?{action:'REOPEN_PAY_RUN',submissionId:selected.id,reason:value,confirmation:'BUKA KEMBALI'}:workflowDialog.kind==='CLOSE'?{action:'CLOSE_PAY_RUN',submissionId:selected.id,confirmation:'TUTUP PERIODE'}:workflowDialog.kind==='CONTROLLER_REVISION'?{action:'TRANSITION_SUBMISSION',submissionId:selected.id,toState:'REVISION_REQUIRED',reviewNote:value}:{action:'CLIENT_REQUEST_PAYROLL_REVISION',submissionId:selected.id,reason:value,confirmation:'MINTA REVISI PAYROLL'};const success=workflowDialog.kind==='DELETE'?'Pay Run dan snapshot input berhasil dihapus':workflowDialog.kind==='REFRESH'?'Pay Run disinkronkan ulang dari master dan perlu difinalisasi ulang':workflowDialog.kind==='REOPEN'?'Periode dibuka kembali untuk revisi':workflowDialog.kind==='CLOSE'?'Periode payroll ditutup':'Permintaan revisi berhasil dikirim';void act(payload,success).then(()=>setSelected(null));}}>Konfirmasi</button></div></section>:null}
      <div className="directory-modal-actions"><button type="button" className="btn" onClick={() => setSelected(null)}>Tutup</button>{['SUPER_ADMIN','PAYROLL_PROCESSOR'].includes(role)&&selected.state==='DRAFT'&&selected.period_status!=='CLOSED'?<button type="button" className="btn btn-danger" onClick={()=>{setWorkflowDialog({kind:'DELETE',value:''});setWorkflowDialogOpen(true);}}>Hapus Pay Run</button>:null}{['SUPER_ADMIN','PAYROLL_PROCESSOR'].includes(role)&&selected.source_mode==='MASTER_CURRENT'&&selected.period_status!=='CLOSED'&&['DRAFT','SUBMITTED','INGESTING','AI_VALIDATING','EXCEPTION_FOUND','CLIENT_ACTION_REQUIRED','CLIENT_RESUBMITTED','REVISION_REQUIRED','CLIENT_REVISION_REQUESTED'].includes(selected.state)?<button type="button" className="btn" onClick={()=>{setWorkflowDialog({kind:'REFRESH',value:''});setWorkflowDialogOpen(true);}}>Hitung ulang dari master</button>:null}{['SUPER_ADMIN','PAYROLL_PROCESSOR'].includes(role)&&selected.input_status==='PENDING'?<button type="button" className="btn" onClick={()=>void act({action:'FINALIZE_PAY_RUN_INPUT',submissionId:selected.id,confirmation:'DATA PAYROLL FINAL'},'Input Pay Run berhasil difinalisasi').then(()=>setSelected(null))}>Finalisasi input</button>:null}{['SUPER_ADMIN','PAYROLL_CONTROLLER'].includes(role)&&selected.period_status==='CLOSED'?<button type="button" className="btn" onClick={()=>{setWorkflowDialog({kind:'REOPEN',value:''});setWorkflowDialogOpen(true);}}>Buka kembali</button>:null}{['SUPER_ADMIN','PAYROLL_CONTROLLER'].includes(role)&&selected.period_status!=='CLOSED'&&selected.state==='COMPLETED'&&selected.payment_status==='COMPLETED'&&selected.reconciliation_status==='MATCHED'&&['ISSUED','PARTIALLY_PAID','PAID'].includes(selected.invoice_status)?<button type="button" className="btn btn-primary" onClick={()=>{setWorkflowDialog({kind:'CLOSE',value:''});setWorkflowDialogOpen(true);}}>Tutup periode</button>:null}{['SUPER_ADMIN','PAYROLL_CONTROLLER'].includes(role)&&selected.state==='CONTROLLER_REVIEW'?<button type="button" className="btn" onClick={()=>{setWorkflowDialog({kind:'CONTROLLER_REVISION',value:''});setWorkflowDialogOpen(true);}}>Minta revisi</button>:null}{clientApprovalPending?<><button type="button" className="btn" onClick={()=>{setWorkflowDialog({kind:'CLIENT_REVISION',value:''});setWorkflowDialogOpen(true);}}>Minta revisi</button><button type="button" className="btn btn-primary" disabled={!confirmed} onClick={()=>void act({action:'CLIENT_APPROVE_PAYROLL',submissionId:selected.id,reviewConfirmed:true,reviewNote:reviewNote||undefined,confirmation:'SETUJUI PAYROLL'},'Payroll berhasil Anda setujui').then(()=>setSelected(null))}>Setujui Payroll</button></>:null}{next ? <button type="button" className="btn btn-primary" disabled={(reviewCheckpoint && !confirmed)||selected.input_status==='PENDING'} onClick={() => {const payload=next==='GENERATE_PAYMENT_INSTRUCTION'?{action:next,submissionId:selected.id}:next.startsWith('ADVANCE_')?{action:'ADVANCE_PAY_RUN',submissionId:selected.id,command:next==='ADVANCE_VALIDATE'?'VALIDATE':'FINALIZE_PAYROLL',reviewConfirmed:true,reviewNote:reviewNote||undefined}:{action:'TRANSITION_SUBMISSION',submissionId:selected.id,toState:next,reviewConfirmed:true,reviewNote:reviewNote||undefined};void act(payload,next==='GENERATE_PAYMENT_INSTRUCTION'?'Payment Instruction draft berhasil dibuat':`${nextAction.label} berhasil`).then(()=>setSelected(null));}}>{nextAction.label}</button> : null}</div>
    </div>
  </div>, document.body)}</>;
}
...
function PayRunLineTable({detail,editable,onEdit}:{detail:any;editable:boolean;onEdit:(line:any,gross:number,deduction:number,included:boolean,changeReason:string)=>Promise<void>}) {
  ...
  const [includedInput,setIncludedInput]=useState(true);
  ...
  useEffect(()=>setPage((current)=>Math.min(current,pages)),[pages]);
  const visible=rows.slice((page-1)*size,page*size);
  const grossNumber=Number(grossInput);
  const deductionNumber=Number(deductionInput);
  const netNumber=grossNumber-deductionNumber;
  const editValid=Boolean(editLine)
    && Number.isSafeInteger(grossNumber)&&grossNumber>=0
    && Number.isSafeInteger(deductionNumber)&&deductionNumber>=0
    && grossNumber>=deductionNumber
  ...
  function openEdit(line:any){
    setEditLine(line);
    setGrossInput(String(line.gross_amount||0));
    setDeductionInput(String(line.deduction_amount||0));
    setIncludedInput(line.included!==0);
    setChangeReason('');
  }
  useEffect(()=>{
    if(!editLine)return;
    const onKey=(event:KeyboardEvent)=>{if(event.key==='Escape'&&!saving){event.preventDefault();setEditLine(null);}};
    document.addEventListener('keydown',onKey);
    const frame=requestAnimationFrame(()=>editDialogRef.current?.querySelector<HTMLElement>('input,textarea,button')?.focus());
    return()=>{cancelAnimationFrame(frame);document.removeEventListener('keydown',onKey);};
  },[editLine,saving]);
  async function saveEdit(){
    if(!editLine||!editValid)return;
    setSaving(true);
    try{
      await onEdit(editLine,grossNumber,deductionNumber,includedInput,changeReason.trim());
      setEditLine(null);
  ...
  <details ref={detailsRef} className="pay-run-lines" open={Boolean(totalIssues)}><summary>{issueFilter==='ALL'?'Preview seluruh penerima':`Filter masalah: ${issueFilter}`} <b>{rows.length}</b></summary><div className="pay-run-line-toolbar"><input value={query} placeholder="Cari nama atau ID karyawan" onChange={(event)=>{setQuery(event.target.value);setPage(1);}}/><select value={issueFilter} onChange={(event)=>{setIssueFilter(event.target.value);setPage(1);}}><option value="ALL">Semua penerima</option><option value="THP">THP kosong/nol</option><option value="GROSS">Gross kosong</option><option value="CONTROL">Control total tidak balance</option><option value="BANK">Rekening belum lengkap</option></select><span>Halaman {Math.min(page,pages)}/{pages}</span></div><div className="pay-run-line-scroll"><table><thead><tr><th>Karyawan</th><th>Gross</th><th>Potongan</th><th>THP</th><th>Variance</th><th>Masalah & rekomendasi</th><th>Aksi</th></tr></thead><tbody>{visible.map((line:any)=>{const issue=issueFor(line);const delta=Number(line.net_amount||0)-Number(line.previous_net||0);return <tr key={line.id} className={issue?'pay-run-line-issue':''}><td><strong>{line.employee_name}</strong><small>{line.employee_code||line.employee_id} · {line.bank_name||'Bank belum ada'} ••••{line.account_last4||'----'}</small></td><td>{formatIDR(Number(line.gross_amount||0))}</td><td>{formatIDR(Number(line.deduction_amount||0))}</td><td><strong>{formatIDR(Number(line.net_amount||0))}</strong></td><td><span className={`pay-run-delta ${delta>0?'up':delta<0?'down':'flat'}`}>{line.variance_type==='NEW'?'NEW':`${delta>0?'+':''}${formatIDR(delta)}`}</span>{line.previous_account_last4&&line.account_last4&&String(line.previous_account_last4)!==String(line.account_last4)?<small>Rekening berubah</small>:null}</td><td>{issue?<><Badge text="CRITICAL"/><small>{issue==='BANK'?'Lengkapi bank dan nomor rekening utama pada master karyawan.':issue==='CONTROL'?'Pastikan Gross - Potongan sama dengan THP.':'Isi gross dan potongan yang benar agar THP dapat dihitung.'}</small></>:<><Badge text="READY"/><small>Data utama siap diproses.</small></>}</td><td>{issue==='BANK'?<a className="btn" href={`?view=employees&employeeQuery=${encodeURIComponent(line.employee_code||line.employee_id)}`}>Perbaiki rekening</a>:editable?<button type="button" className="btn btn-primary" onClick={()=>openEdit(line)}>Edit nominal</button>:<span>Review only</span>}</td></tr>})}</tbody></table></div><div className="control-pagination"><span>{rows.length} hasil · {totalIssues} masalah ditemukan</span><div><button className="btn" aria-label="Halaman penerima sebelumnya" disabled={page<=1} onClick={()=>setPage((value)=>Math.max(1,value-1))}>←</button><button className="btn" aria-label="Halaman penerima berikutnya" disabled={page>=pages} onClick={()=>setPage((value)=>Math.min(pages,value+1))}>→</button></div></div></details>
  {editLine?createPortal(<div className="directory-modal-backdrop pay-run-edit-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget&&!saving)setEditLine(null);}}><div ref={editDialogRef} className="directory-modal pay-run-edit-modal" role="dialog" aria-modal="true" aria-label={`Edit nominal ${editLine.employee_name}`}><div className="directory-modal-title"><div><span>PAYROLL LINE EDIT</span><h3>{editLine.employee_name}</h3></div><button type="button" aria-label="Tutup edit nominal" disabled={saving} onClick={()=>setEditLine(null)}>✕</button></div><div className="pay-run-edit-before"><div><span>Gross sebelumnya</span><strong>{formatIDR(Number(editLine.gross_amount||0))}</strong></div><div><span>Potongan sebelumnya</span><strong>{formatIDR(Number(editLine.deduction_amount||0))}</strong></div><div><span>THP sebelumnya</span><strong>{formatIDR(Number(editLine.net_amount||0))}</strong></div></div><div className="directory-form-grid"><label>Gross<input inputMode="numeric" value={grossInput} onChange={(event)=>setGrossInput(event.target.value.replace(/[^0-9]/g,''))}/></label><label>Potongan<input inputMode="numeric" value={deductionInput} onChange={(event)=>setDeductionInput(event.target.value.replace(/[^0-9]/g,''))}/></label><label>THP hasil<input value={Number.isFinite(netNumber)?formatIDR(Math.max(0,netNumber)):'-'} readOnly /></label><label className="pay-run-include-toggle"><input type="checkbox" checked={includedInput} onChange={(event)=>setIncludedInput(event.target.checked)}/><span>Masukkan karyawan pada payroll ini</span></label></div><label className="pay-run-edit-reason"><span>Alasan perubahan</span><textarea rows={3} maxLength={500} value={changeReason} onChange={(event)=>setChangeReason(event.target.value)} placeholder="Minimal 10 karakter; alasan ini disimpan dalam audit trail."/><small>{changeReason.trim().length}/500</small></label>{grossNumber<deductionNumber?<div className="app-notice-bubble app-notice-error"><strong>Nominal tidak valid</strong><span>Gross harus lebih besar atau sama dengan potongan.</span></div>:null}<div className="pay-run-edit-summary"><span>THP baru</span><strong>{editValid?formatIDR(netNumber):'-'}</strong><small>Δ {editValid?formatIDR(netNumber-Number(editLine.net_amount||0)):'-'}</small></div><div className="directory-modal-actions"><button type="button" className="btn" disabled={saving} onClick={()=>setEditLine(null)}>Batal</button><button type="button" className="btn btn-primary" disabled={!editValid||saving} onClick={()=>void saveEdit()}>{saving?'Menyimpan…':'Simpan perubahan'}</button></div></div></div>,document.body):null}</>;
}
  ...
  return <div style={{ display:'grid', gap:16 }}>
    ...
    {detail ? createPortal(<div className="directory-modal-backdrop pi-detail-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)setDetail(null);}}><div className="directory-modal pi-detail-modal" role="dialog" aria-modal="true" aria-label="Detail Payment Instruction">
      ...
      <div className="pi-detail-body">
        ...
        <section className="pi-approval-section"><div className="pi-section-heading"><div><span>GOVERNANCE</span><h4>Approval trail</h4></div><small>{detail.approvals?.length || 0} aktivitas</small></div>{detail.approvals?.length ? <div className="pi-approval-list">{detail.approvals.map((approval)=><div key={approval.id}><i>✓</i><div><strong>{String(approval.status || '').replaceAll('_',' ')}</strong><span>{approval.approver_email || approval.approver_user_id || 'System'} · {date(approval.created_at)}</span></div></div>)}</div> : <p className="directory-hint">Belum ada approval yang tercatat.</p>}</section>
        <section className="pi-approval-section"><div className="pi-section-heading"><div><span>OPERATIONAL AUDIT</span><h4>Activity trail</h4></div><small>{detail.activity?.length || 0} event terakhir</small></div>{detail.activity?.length ? <div className="pi-approval-list">{detail.activity.slice(0,20).map((item)=><div key={item.id}><i>•</i><div><strong>{paymentActivityLabel(item.action)}</strong><span>{item.username || 'System'} · {date(item.timestamp)}</span>{item.detail?<small>{item.detail}</small>:null}</div></div>)}</div> : <p className="directory-hint">Belum ada aktivitas operasional yang tercatat.</p>}</section>
      </div>
