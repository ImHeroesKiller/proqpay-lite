  ...
  return <div style={{ display:'grid', gap:16 }}>
    <CardTable headers={['Instruction / Periode','Nilai','Status','Dibuat','Aksi']} rows={instructions.map((r) => {
      ...
      if (r.status === 'PAYMENT_INSTRUCTION_READY') action = ['SUPER_ADMIN','PAYROLL_PROCESSOR'].includes(role)
        ...
        : canApprove ? <button style={actionButton} onClick={() => void act({action:'OPEN_PAYMENT_REVIEW',paymentInstructionId:r.id,confirmation:'MULAI REVIEW PI'},'PI siap direview dan diputuskan Controller')}>Mulai review</button>
      ...
      return [<div key="id"><strong>{r.document_no || r.client_name || r.id}{Number(r.revision_no||0)>1?` · Revisi ${Number(r.revision_no)-1}`:''}</strong><small style={small}>{r.client_name || '-'} · Payroll {r.payroll_period || '-'} · Bayar {r.payment_period || r.payroll_period || '-'}</small>{r.rejection_reason?<small style={{...small,color:'#b91c1c'}}>Alasan reject: {r.rejection_reason}</small>:null}</div>, formatIDR(Number(r.expected_total || 0)), <Badge key="status" text={r.status} />, date(r.created_at), <span key="action">{action}</span>];
    })} />
    ...
    {detail ? createPortal(<div className="directory-modal-backdrop pi-detail-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)setDetail(null);}}><div className="directory-modal pi-detail-modal" role="dialog" aria-modal="true" aria-label="Detail Payment Instruction">
      ...
      <footer className="pi-detail-footer">
        <div className="pi-export-actions"><a className="btn" href={`/api/payment-instruction-export?id=${encodeURIComponent(detail.paymentInstruction.id)}&format=PDF`} target="_blank" rel="noreferrer">Unduh PDF resmi</a>{detail.paymentInstruction.status==='PAYMENT_INSTRUCTION_READY'&&canApprove?<button type="button" className="btn btn-primary" onClick={()=>void act({action:'OPEN_PAYMENT_REVIEW',paymentInstructionId:detail.paymentInstruction.id,confirmation:'MULAI REVIEW PI'},'PI siap direview dan diputuskan Controller').then(()=>setDetail(null))}>Mulai review</button>:null}{detail.paymentInstruction.status==='PAYMENT_APPROVAL_PENDING'?<button type="button" className="btn btn-primary" disabled={sendingPi} onClick={()=>void sendPiToClient()}>{sendingPi?'Menyiapkan email…':'Kirim ke klien'}</button>:null}{['APPROVED_FOR_PAYMENT','DISBURSEMENT_PROCESSING','PROOF_UPLOADED','COMPLETED'].includes(detail.paymentInstruction.status) ? ['BCA','MANDIRI','BRI','BNI','CUSTOM'].map((format)=><a key={format} className="btn" href={`/api/payment-instruction-export?id=${encodeURIComponent(detail.paymentInstruction.id)}&format=${format}`}>{format}</a>) : null}</div>
