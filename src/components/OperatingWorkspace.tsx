...
function Submissions({ rows, role, act }: { rows: any[]; role: string; act: (p: Record<string, unknown>, s: string) => Promise<void> }) {
  ...
  const actionName = (state:string) => ({ DRAFT:'Submit ke proses validasi', SUBMITTED:'Periksa submission', INGESTING:'Mulai validasi', AI_VALIDATING:'Review hasil validasi', EXCEPTION_FOUND:'Minta perbaikan klien', VALIDATED:'Standardisasi data', STANDARDIZED:'Kirim ke Controller', CONTROLLER_REVIEW:'Setujui data payroll', DATA_APPROVED:'Siapkan payment', PAYROLL_FINALIZED:'Terbitkan PI', PAYMENT_INSTRUCTION_READY:'Buat payment instruction', CLIENT_ACTION_REQUIRED:'Kirim perbaikan' }[state] || 'Lihat detail');
  ...
  const flowStates=selected.service_tier==='TIER_1_PAYMENT_PROCESSING'
    ? ['DRAFT','SUBMITTED','AI_VALIDATING','VALIDATED','STANDARDIZED','CONTROLLER_REVIEW','DATA_APPROVED','PAYMENT_INSTRUCTION_READY']
    : ['DRAFT','SUBMITTED','INGESTING','AI_VALIDATING','VALIDATED','STANDARDIZED','CONTROLLER_REVIEW','DATA_APPROVED','PAYROLL_FINALIZED','PAYMENT_INSTRUCTION_READY'];
  const currentFlowIndex=Math.max(0,flowStates.indexOf(selected.state));
  ...
  return <>{table}{createPortal(<div className="directory-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
    <div className="directory-modal payroll-review-modal" role="dialog" aria-modal="true" aria-label="Review payroll submission">
      ...
      <div className="pay-run-flow-guide" aria-label="Tahapan Pay Run menuju Payment Instruction">{flowStates.map((state,index)=><div key={state} className={index<currentFlowIndex?'done':index===currentFlowIndex?'current':''}><i>{index<currentFlowIndex?'✓':index+1}</i><span>{state==='DRAFT'?'Input':state==='AI_VALIDATING'?'Validasi':state==='VALIDATED'?'Valid':state==='STANDARDIZED'?'Standar':state==='CONTROLLER_REVIEW'?'Controller':state==='DATA_APPROVED'?'Approved':state==='PAYROLL_FINALIZED'?'Final payroll':state==='PAYMENT_INSTRUCTION_READY'?'Siap PI':'Submitted'}</span></div>)}</div>
