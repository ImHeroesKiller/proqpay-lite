...
function Submissions({ rows, role, act }: { rows: any[]; role: string; act: (p: Record<string, unknown>, s: string) => Promise<void> }) {
  ...
  function nextFor(row:any) {
    ...
    if (['SUPER_ADMIN','PAYROLL_CONTROLLER'].includes(role) && ['CONTROLLER_REVIEW','DATA_APPROVED','PAYROLL_FINALIZED'].includes(row.state)) return 'ADVANCE_APPROVE';
    ...
    if (['SUPER_ADMIN','PAYROLL_PROCESSOR'].includes(role) && ['VALIDATED','STANDARDIZED'].includes(row.state)) return 'ADVANCE_APPROVAL';
  ...
  const actionName = (state:string) => ({ DRAFT:'Validasi Pay Run', SUBMITTED:'Validasi Pay Run', INGESTING:'Selesaikan validasi', AI_VALIDATING:'Selesaikan validasi', EXCEPTION_FOUND:'Minta perbaikan klien', CLIENT_RESUBMITTED:'Validasi ulang', REVISION_REQUIRED:'Validasi ulang', VALIDATED:'Kirim ke Controller', STANDARDIZED:'Kirim ke Controller', CONTROLLER_REVIEW:'Setujui payroll', DATA_APPROVED:'Selesaikan approval', PAYROLL_FINALIZED:'Siapkan Payment Instruction', PAYMENT_INSTRUCTION_READY:'Terbitkan Payment Instruction', CLIENT_ACTION_REQUIRED:'Kirim perbaikan' }[state] || 'Lihat detail');
  ...
  const flowStates=['Input','Validasi','Approval','Payment Instruction'];
  const currentFlowIndex=['DRAFT'].includes(selected.state)?0
    : ['SUBMITTED','INGESTING','AI_VALIDATING','EXCEPTION_FOUND','CLIENT_ACTION_REQUIRED','CLIENT_RESUBMITTED','REVISION_REQUIRED','VALIDATED'].includes(selected.state)?1
      : ['STANDARDIZED','CONTROLLER_REVIEW','DATA_APPROVED','PAYROLL_FINALIZED'].includes(selected.state)?2:3;
  const reviewCheckpoint = next === 'ADVANCE_APPROVAL' || next === 'ADVANCE_APPROVE';
