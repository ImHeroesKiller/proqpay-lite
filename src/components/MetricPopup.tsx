...
export default function MetricPopup({ type, db, onClose }: MetricPopupProps) {
  ...
  switch (type) {
    case 'employees':
      title = 'All Employees';
      headers = ['ID', 'Name', 'Company', 'Position', 'Status', 'Region', 'Salary'];
      rows = (db.employees || []).map((e: any) => [
        e.id, e.name, e.company, e.position || '-', e.status, e.region || '-', formatIDR(e.salaryGross)
      ]);
      break;
    case 'clients':
      title = 'All Clients';
      headers = ['ID', 'Name', 'NPWP', 'PIC', 'Phone', 'Payroll Type'];
      rows = (db.companies || []).map((c: any) => [
        c.id, c.name, c.npwp || '-', c.pic || '-', c.phone || '-', c.payrollType
      ]);
      break;
    case 'payroll':
      title = 'Payroll Details';
      headers = ['ID', 'Period', 'Status', 'Employees', 'Gross', 'Net'];
      rows = (db.payrolls || []).map((p: any) => [
        p.id, p.period, p.status,
    ...
    case 'outstanding':
      ...
      headers = ['ID', 'Company', 'Invoice', 'Amount', 'Status', 'Days Overdue'];
      rows = (db.arMonitor || []).map((a: any) => [
        a.id, a.company, a.invoiceId,
        formatIDRShort(a.amount), a.status, String(a.daysOverdue || 0)
      ]);
      break;
  }
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '0 0 12px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12.5px' }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={headers.length} style={{ padding: '24px', textAlign: 'center', color: 'var(--text3)' }}>
                    No results
                  </td>
                </tr>
              ) : filtered.map((r, i) => (
