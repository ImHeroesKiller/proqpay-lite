...
export default function PaymentGatewayExecutionActions({ paymentInstructionId, canExecuteGateway, onManualProof, onChanged }: Props) {
  const [runtime, setRuntime] = useState<Runtime>({ seamless:null, hosted:null, transaction:null, items:[], session:null });
  ...
  return <div style={{ display:'grid', gap:6, minWidth:190 }}>
    <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
      ...
      {canExecuteGateway && isE2Pay && e2payFailed > 0 && e2payUnresolved === 0 && e2payReady === 0 ? <button className="btn" type="button" disabled={Boolean(busy)} onClick={() => void retryFailedE2Pay()}>{busy === 'retry-failed' ? 'Retry…' : `Retry ${e2payFailed} Gagal`}</button> : null}
    ...
    {isE2Pay && e2payFailed > 0 && e2payUnresolved === 0 && e2payReady === 0 && e2paySucceeded < runtime.items.length ? <small style={{ color:'#b45309' }}>Ada beneficiary gagal. Gunakan retry terkontrol hanya untuk item yang provider nyatakan gagal final.</small> : null}
