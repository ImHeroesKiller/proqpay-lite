...
function Badge({text}:any){const good=/PAID|ISSUED|APPROVED|CURRENT|NON_PKP/.test(text);const bad=/REJECT|OVERDUE|DISPUT/.test(text);const color=good?'#059669':bad?'#dc2626':'#4f46e5';return <span style={{display:'inline-block',color,background:`${color}14`,borderRadius:999,padding:'4px 8px',fontWeight:700,fontSize:9.5,whiteSpace:'nowrap'}}>{String(text||'-').replaceAll('_',' ')}</span>}
