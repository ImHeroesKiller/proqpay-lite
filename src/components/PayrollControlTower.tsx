...
export default function PayrollControlTower({actor,period,onNavigate}:Props) {
  ...
  return <section className={`control-tower${simplifiedInternal?' control-tower-simple':''}`} aria-busy={loading||refreshing}>
    ...
    {error?<div className="app-notice-bubble app-notice-error" role="alert"><strong>Dashboard gagal dimuat</strong><span>{error}</span></div>:null}
    ...
    {loading?<div className="card control-loading" role="status" aria-live="polite">Menyiapkan payroll control tower…</div>:<>
      ...
      <div className="control-kpis" aria-label="KPI dashboard payroll">
        {simplifiedInternal ? <>
          ...
          <Kpi label="E2Pay balance" value={gatewayBalanceLoading?'…':gatewayAccount?.balance!==null&&gatewayAccount?.balance!==undefined?formatIDRShort(Number(gatewayAccount.balance)):'—'} note={gatewayBalanceError||gatewayAccount?.refreshedAt?`${gatewayAccount?.environment||'E2PAY'} · update ${gatewayAccount?.refreshedAt?new Date(gatewayAccount.refreshedAt).toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'}):'—'}`:'Saldo merchant account'} tone="green" icon={<IconWallet />} onClick={()=>openContext('integrations')} />
      ...
      <div className={simplifiedInternal?'':'control-priority-grid'}>
        <section className="card action-center"><PanelTitle eyebrow="PRIORITY QUEUE" title={simplifiedInternal?'My Work':'Action Center'} meta={`${actions.length} tindakan`} />
          <div className="action-list">{actions.length?actions.slice(0,simplifiedInternal?10:8).map((item)=><button type="button" key={item.id} onClick={()=>openContext(item.view,item.submissionId)} aria-label={`${item.action} untuk ${item.client}`}><i className={`action-tone ${item.tone}`} /><span><strong>{item.client}</strong><small>{item.detail}</small></span><b>{item.amount?formatIDRShort(item.amount):'-'}</b><em>{item.action} →</em></button>):<Empty text={activeFilterCount?'Tidak ada tindakan sesuai filter aktif.':'Tidak ada tindakan yang membutuhkan Anda saat ini.'} />}</div>
        </section>
      ...
      <section className="card portfolio-panel"><PanelTitle eyebrow={simplifiedInternal?'ALL PAYROLL':'PORTFOLIO MONITORING'} title={simplifiedInternal?'Payroll Overview':'Pay Run Portfolio'} meta={`${visible.length} record`} />
        <div className="portfolio-table-wrap"><table className="portfolio-table dashboard-responsive-table">
          <caption className="visually-hidden">{simplifiedInternal?'Daftar payroll dan tindakan berikutnya':'Portfolio Pay Run sesuai filter dashboard'}</caption>
          <thead>{simplifiedInternal
          ? <tr><th scope="col">Klien / Project</th><th scope="col">Periode</th><th scope="col">Stage</th><th scope="col">Net / THP</th><th scope="col">Next action</th></tr>
          : <tr><th scope="col">Klien / Project</th><th scope="col">Periode</th><th scope="col">Tier</th><th scope="col">Penerima</th><th scope="col">Net / THP</th><th scope="col">Blocker</th><th scope="col">Current stage</th><th scope="col">Next action</th></tr>}</thead><tbody>{pageRows.map((row)=>simplifiedInternal
            ? <tr key={row.id}><td data-label="Klien / Project"><strong>{row.client_name||row.client_id}</strong><small>{row.project_name||row.id}</small></td><td data-label="Periode">{row.period}<small>Bayar {row.payment_period||row.period}</small></td><td data-label="Stage"><span className="stage-pill">{row.business.label}</span><small>{statusLabel(row.business.status)}</small></td><td data-label="Net / THP"><strong>{formatIDR(Number(row.total_net||0))}</strong>{Number(row.blocking_count||0)?<small>{row.blocking_count} blocker</small>:null}</td><td data-label="Next action"><button type="button" onClick={()=>openContext(row.nextAction.view as AppView,String(row.id))}>{row.nextAction.label} →</button><small>{row.nextAction.actionable?'Action required':'Monitor only'}</small></td></tr>
            : <tr key={row.id}><td data-label="Klien / Project"><strong>{row.client_name||row.client_id}</strong><small>{row.project_name||row.id}</small></td><td data-label="Periode">{row.period}<small>Bayar {row.payment_period||row.period}</small></td><td data-label="Tier">{statusLabel(row.service_tier).replace('TIER 1 ','T1 · ').replace('TIER 2 ','T2 · ').replace('TIER 3 ','T3 · ')}</td><td data-label="Penerima">{Number(row.employee_count||0).toLocaleString('id-ID')}</td><td data-label="Net / THP"><strong>{formatIDR(Number(row.total_net||0))}</strong></td><td data-label="Blocker"><span className={Number(row.blocking_count||0)?'table-blocker':'table-clear'}>{Number(row.blocking_count||0)}</span></td><td data-label="Current stage"><span className="stage-pill">{row.business.label}</span><small>{statusLabel(row.state)}</small></td><td data-label="Next action"><button type="button" onClick={()=>openContext(row.nextAction.view as AppView,String(row.id))}>{row.nextAction.label} →</button><small>{row.nextAction.actionable?'Action required':'No action required'}</small></td></tr>)}</tbody></table>{!pageRows.length?<Empty text={activeFilterCount?'Tidak ada payroll sesuai filter aktif. Coba reset filter atau ubah periode.':'Belum ada payroll untuk periode ini.'} />:null}</div>
        <div className="control-pagination"><span>Halaman {Math.min(page,pageCount)} dari {pageCount}</span><div><button className="btn" aria-label="Halaman sebelumnya" disabled={page<=1} onClick={()=>setPage((value)=>value-1)}>←</button><button className="btn" aria-label="Halaman berikutnya" disabled={page>=pageCount} onClick={()=>setPage((value)=>value+1)}>→</button></div></div>
      </section>
...
function Empty({text}:{text:string}) { return <div className="control-empty" role="status">{text}</div>; }
