...
const PIPELINE = [
  {label:'Intake',states:['DRAFT','SUBMITTED','INGESTING']},
  {label:'Validation',states:['AI_VALIDATING','VALIDATED','EXCEPTION_FOUND','CLIENT_ACTION_REQUIRED','CLIENT_RESUBMITTED']},
  {label:'Processor',states:['STANDARDIZED','REVISION_REQUIRED']},
  {label:'Controller',states:['CONTROLLER_REVIEW','DATA_APPROVED','PAYROLL_FINALIZED']},
  {label:'PI Approval',states:['PAYMENT_INSTRUCTION_READY','PAYMENT_APPROVAL_PENDING']},
  {label:'Payment',states:['APPROVED_FOR_PAYMENT','DISBURSEMENT_PROCESSING','PROOF_UPLOADED']},
  {label:'Reconciled',states:['COMPLETED','RECONCILIATION']},
];
...
export default function PayrollControlTower({actor,period,onNavigate}:Props) {
  ...
  const awaitingApproval=visible.filter((row)=>['CONTROLLER_REVIEW','PAYMENT_APPROVAL_PENDING','PAYMENT_INSTRUCTION_READY'].includes(row.state)).length;
  ...
  const actions=useMemo(()=>{
    ...
    visible.forEach((row)=>{
      ...
      else if(['CONTROLLER_REVIEW','PAYMENT_APPROVAL_PENDING','PAYMENT_INSTRUCTION_READY'].includes(row.state)) list.push({id:`approve-${row.id}`,tone:'warning',title:'Menunggu approval',detail:`${stageFor(row.state)} · Payroll ${row.period}`,client:row.client_name||row.client_id,amount:Number(row.total_net||0),action:'Review & approve'});
  ...
  },[visible,visibleReconciliations]);
  ...
  const pipeline=PIPELINE.map((stage)=>({label:stage.label,rows:visible.filter((row)=>stage.states.includes(row.state))}));
  ...
  return <section className="control-tower">
    ...
    {loading?<div className="card control-loading">Menyiapkan payroll control tower…</div>:<>
      ...
      <section className="card pipeline-panel"><PanelTitle eyebrow="END-TO-END WORKFLOW" title="Payroll Pipeline" meta={`${visible.length} pay run`} /><div className="pipeline-grid">{pipeline.map((stage,index)=><button type="button" key={stage.label} onClick={()=>onNavigate('operations')}><span>{index+1}</span><strong>{stage.rows.length}</strong><small>{stage.label}</small><em>{formatIDRShort(stage.rows.reduce((sum,row)=>sum+Number(row.total_net||0),0))}</em></button>)}</div></section>
