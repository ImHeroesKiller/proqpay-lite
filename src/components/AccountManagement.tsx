...
type Project = { id: string; client_id: string; code: string; name: string; status: string };
...
export default function AccountManagement() {
  ...
  const [draft, setDraft] = useState({ name: '', email: '', role: 'PAYROLL_PROCESSOR' as AppRole, clientIds: [] as string[], projectIds: [] as string[], paymentApprover: false });
  ...
  async function createAccount() {
    ...
    try {
      ...
      setDraft({ name: '', email: '', role: 'PAYROLL_PROCESSOR', clientIds: [], projectIds: [], paymentApprover: false });
      await load();
    } catch (cause) {
  ...
  async function saveAccount(user: Account) {
    ...
    try {
      await accountRequest({ action: 'UPDATE', userId: user.id, name: user.name, role: user.role, status: user.status, paymentApprover: user.paymentApprover, clientIds: user.clientIds, projectIds: user.projectIds });
      await load();
    } catch (cause) {
...
function AccessScopes({ clients, projects, clientIds, projectIds, onChange, compact = false }: { clients: Client[]; projects: Project[]; clientIds: string[]; projectIds: string[]; onChange: (clientIds: string[], projectIds: string[]) => void; compact?: boolean }) {
