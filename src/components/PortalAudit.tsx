    ...
    {selected?<div className="pa-drawer-backdrop" role="presentation" onMouseDown={e=>{if(e.currentTarget===e.target)setSelected(null)}}>
      <aside className="pa-drawer" role="dialog" aria-modal="true" aria-labelledby="pa-detail-title">
        <div className="pa-drawer-head">
          <div><span className="page-eyebrow">Audit detail</span><h2 id="pa-detail-title" style={{margin:"4px 0"}}>{"created_at" in selected?"Login activity":actionLabel(selected.action)}</h2><div style={{fontSize:11,color:"var(--text3)"}}>{"created_at" in selected?selected.id:selected.action}</div></div>
